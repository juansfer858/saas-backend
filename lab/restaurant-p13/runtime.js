...
const LOCAL_LOGIN_MARKER = 'VANTIX_RESTAURANT_P13_LOCAL_LOGIN_V1';
...
<title>VantixGC Restaurante · Acceso local</title>
<style>
:root{font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#18221d;background:#f4f4f5}*{box-sizing:border-box}body{margin:0;min-height:100vh;display:grid;place-items:center;padding:24px;background:linear-gradient(145deg,#eef3f0,#f7f7f6)}.card{width:min(430px,100%);background:#fff;border:1px solid #dde2df;border-radius:18px;padding:28px;box-shadow:0 18px 50px rgba(24,34,29,.10)}.brand{display:flex;gap:11px;align-items:center;margin-bottom:24px}.logo{width:38px;height:38px;border-radius:11px;background:#0d6b43;color:#fff;display:grid;place-items:center;font-weight:900}.brand b{display:block}.brand small{display:block;color:#61706a;margin-top:2px}.badge{display:inline-flex;padding:5px 9px;border-radius:999px;background:#ddf3e8;color:#0d6b43;font-size:12px;font-weight:800;margin-bottom:12px}h1{font-size:24px;margin:0 0 7px}p{color:#61706a;margin:0 0 22px}.field{margin:14px 0}.field label{display:block;font-size:13px;font-weight:750;margin-bottom:6px}.field input{width:100%;padding:11px 12px;border:1px solid #cfd7d2;border-radius:10px;font:inherit}.btn{width:100%;border:0;border-radius:10px;background:#0d6b43;color:#fff;padding:12px 14px;font:inherit;font-weight:800;cursor:pointer}.btn:disabled{opacity:.65}.error{margin:12px 0;padding:10px 12px;border-radius:9px;background:#fff1f0;color:#b42318;border:1px solid #fecaca;font-size:13px}.meta{margin-top:14px;color:#61706a;font-size:12px}
</style></head><body data-p13-local-login="${LOCAL_LOGIN_MARKER}"><form class="card" id="loginForm">
<div class="brand"><div class="logo">V</div><div><b>VantixGC Restaurante</b><small>P13 · operación local aislada</small></div></div>
<div class="badge">LOCAL · 127.0.0.1:8790</div><h1>Ingresar al restaurante</h1><p>Esta sesión existe únicamente en este PC piloto.</p>
<div id="errorBox"></div><div class="field"><label>Correo electrónico</label><input name="email" type="email" value="admin@demo-restaurante.vantixgc.com" required autocomplete="username"></div>
...
<button class="btn" id="submit" type="submit">Ingresar</button><div class="meta">Tenant local: demo-restaurante · Edge productivo 8788 permanece separado.</div></form>
<script>
(()=>{'use strict';const marker=${JSON.stringify(LOCAL_LOGIN_MARKER)};const tenant=${tenant};const entry=${entry};const key='vantixgc_core_session_v1';document.documentElement.dataset.p13LocalLogin=marker;try{const existing=JSON.parse(localStorage.getItem(key)||'null');if(existing?.token&&existing?.subdomain===tenant){location.replace(entry);return}}catch{}const form=document.getElementById('loginForm');const box=document.getElementById('errorBox');const submit=document.getElementById('submit');const esc=v=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]));form.addEventListener('submit',async e=>{e.preventDefault();box.innerHTML='';submit.disabled=true;submit.textContent='Ingresando…';const data=new FormData(form);try{const loginResponse=await fetch('/api/v1/auth/login',{method:'POST',cache:'no-store',headers:{Accept:'application/json','Content-Type':'application/json','x-tenant-subdomain':tenant},body:JSON.stringify({email:String(data.get('email')||'').trim(),password:String(data.get('password')||'')})});let login={};try{login=await loginResponse.json()}catch{}if(!loginResponse.ok||!login?.data?.token)throw new Error(login?.error?.message||login?.message||'Credenciales inválidas');const token=login.data.token;const sessionResponse=await fetch('/api/v1/auth/session',{cache:'no-store',headers:{Accept:'application/json',Authorization:'Bearer '+token,'x-tenant-subdomain':tenant}});let session={};try{session=await sessionResponse.json()}catch{}if(!sessionResponse.ok||!session?.data?.tenant||!session?.data?.user)throw new Error(session?.error?.message||'No fue posible abrir la sesión local');localStorage.setItem(key,JSON.stringify({token,subdomain:tenant,tenant:session.data.tenant,user:session.data.user}));location.replace(entry)}catch(error){box.innerHTML='<div class="error">'+esc(error?.message||'No fue posible iniciar sesión')+'</div>';submit.disabled=false;submit.textContent='Ingresar'}})})();
</script></body></html>`;
}
...
function mutationBoundaryForRequest(method, rawPath) {
  ...
  if (verb === 'POST' && /^\/api\/v1\/restaurante\/mesas\/[^/]+\/cerrar-vacia-v21$/.test(pathname)) return 'RESTAURANT_TABLE_VISIT';
