  ...
  try {
    ...
    for (const operation of pending) {
      try {
        ...
        if (result?.ok) store.markSynced(operation.id);
        else store.markFailed(operation.id, `${result?.code || 'EDGE_SYNC_FAILED'}: ${result?.message || 'Sin detalle'}`);
      } catch (error) {
        runtime.connected = false;
        runtime.lastError = error.message;
    ...
    if (store.pendingCount() === 0 && !runtime.revoked) {
      ...
      try { await refreshBootstrap(true); } catch (error) { runtime.connected = false; runtime.lastError = error.message; }
    }
  } finally {
...
async function createLocalSale(input) {
  ...
  const cashReceived = Number(input.cashReceived ?? calculation.total);
  if (cashReceived < calculation.total) throw new Error('El efectivo recibido no cubre el total de la venta');
  const payload = {
    terceroId: input.terceroId || calculation.catalog.edge?.defaultCustomerId || null,
    cajaBancoId: input.cajaBancoId || calculation.catalog.edge?.defaultCashAccountId || null,
    formaPago: 'EFECTIVO',
  ...
  store.saveLocalSale({ id: saleId, operationId, localNumber, total: calculation.total, cashReceived, payload, createdAt });
  for (const [productId, delta] of calculation.deltas.entries()) store.adjustStock(productId, delta);
  const print = await printReceipt({ id: saleId, localNumber, cashReceived }, calculation).catch((error) => ({ ok: false, error: error.message, code: error.code || 'PRINT_ERROR' }));
  void flushQueue();
  return { id: saleId, operationId, localNumber, total: calculation.total, cashReceived, change: Number((cashReceived - calculation.total).toFixed(2)), print, mode: runtime.connected ? 'CONNECTED' : 'OFFLINE', pending: store.pendingCount() };
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/sync-now') {
      try { await flushQueue(); await refreshBootstrap(true); } catch (error) { runtime.connected = false; runtime.lastError = error.message; }
