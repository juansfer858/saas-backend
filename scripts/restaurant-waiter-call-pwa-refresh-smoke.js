...
async function main() {
  ...
  assert.match(refreshRoutes, /v16-autopedido-code-v22-electronic-payment/);
  assert.match(refreshRoutes, /\/app\/centro-de-control\/mesero/);
  assert.match(refreshRoutes, /\/app\/centro-de-control\/sw\.js/);
  assert.match(publicRoot, /restaurantElectronicPaymentPublicRouter/);
  assert.match(publicRoot, /router\.use\(restaurantElectronicPaymentPublicRouter\);[\s\S]*router\.use\(restaurantWaiterCallRefreshPublicRouter\);[\s\S]*router\.use\(restaurantWaiterCallPublicRouter\)/);
  ...
  await withServer(async (baseUrl) => {
    ...
    assert.match(pwa, /<script[^>]+restaurant-waiter-runtime-v7\.js\?v=waiter-runtime-v22-electronic-payment[^>]*><\/script>/);
    ...
    assert.equal(swResponse.headers.get('x-vantixgc-waiter-payment'), 'v22-electronic');
    assert.match(sw, /vantixgc-waiter-shell-v14-review-hard-gate-v16-autopedido-code-v22-electronic-payment/);
    assert.match(sw, /restaurant-waiter-runtime-v7\.js\?v=waiter-runtime-v22-electronic-payment/);
    ...
    const runtimeResponse = await fetch(`${baseUrl}/app/restaurant-waiter-runtime-v7.js?v=waiter-runtime-v22-electronic-payment`, { cache:'no-store' });
  ...
  console.log('RESTAURANT WAITER CALL + ACCOUNT REQUEST + ELECTRONIC PAYMENT V22 SMOKE OK');
}
