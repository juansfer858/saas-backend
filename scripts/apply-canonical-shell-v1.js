...
const smokePath = 'scripts/core-canonical-shell-smoke.js';
fs.writeFileSync(smokePath, `const assert = require('node:assert/strict');\nconst fs = require('node:fs');\nconst { app } = require('../src/app');\n\nconst routes = ['/app/dashboard','/app/ventas','/app/compras','/app/inventario','/app/tesoreria','/app/cartera','/app/terceros','/app/contabilidad','/app/configuracion','/app/configuracion-avanzada'];\n\nasync function main(){\n  const server=app.listen(0,'127.0.0.1');\n  await new Promise(r=>server.once('listening',r));\n  const base='http://127.0.0.1:'+server.address().port;\n  try{\n    for(const route of routes){\n      const response=await fetch(base+route);\n      const html=await response.text();\n      assert.equal(response.status,200,route);\n      assert.equal(response.headers.get('x-vantixgc-tenant-shell'),'core-shell-v1',route);\n      assert.match(html,/id=\\"core-shell-bootstrap\\"/,route);\n      assert.match(html,/data-core-shell-topbar=\\"v1\\"/,route);\n      assert.match(html,/window\\.VantixGCCoreShell\\?\\.topbarHtml/,route);\n      assert.ok(!html.includes('<header class=\\"top\\">'),route+' cannot keep legacy top');\n      assert.ok(!html.includes('<header class=\\"topbar\\">'),route+' cannot keep legacy topbar');\n    }\n    const css=fs.readFileSync('src/web/super-core-workspace-v6.css','utf8');\n    assert.match(css,/\\.app\\{grid-template-columns:250px minmax\\(0,1fr\\)!important\\}/);\n    assert.match(css,/\\.core-shell-topbar\\{/);\n    assert.match(css,/\\.core-shell-user-copy\\{/);\n    assert.match(css,/\\.pagehead,\\.head\\{/);\n    console.log('SUPER CORE CANONICAL SHELL V1 ALL MODULES SMOKE OK');\n  }finally{await new Promise(r=>server.close(r));}\n}\nmain().catch(e=>{console.error(e);process.exitCode=1});\n`);

const workflowPath = '.github/workflows/super-core-ci.yml';
let workflow = fs.readFileSync(workflowPath, 'utf8');
workflow = replaceOnce(
  workflow,
  "      - name: Validate global Panel and Sales UI\n        run: node scripts/panel-ui-smoke.js\n",
  "      - name: Validate global Panel and Sales UI\n        run: node scripts/panel-ui-smoke.js\n\n      - name: Validate canonical Super Core shell across all modules\n        run: node scripts/core-canonical-shell-smoke.js\n",
  'CI canonical shell step'
);
