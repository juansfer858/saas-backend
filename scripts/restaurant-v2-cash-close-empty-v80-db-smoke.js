...
async function main() {
  ...
  const eligible = await live.liveDetail(demo.tenantId, admin, table.id);
  assert.equal(eligible.open, true);
  assert.equal(eligible.session.state, 'CUENTA_PEDIDA');
  assert.equal(Number(eligible.sale.total), 0);
  assert.equal(eligible.items.length, 0);
  assert.equal(eligible.canCloseEmptyFromControlCenter, true, 'Caja debe recibir elegibilidad real del backend');
  ...
  const closed = await live.closeEmptyFromControlCenter(demo.tenantId, admin, table.id);
  ...
  console.log(JSON.stringify({
    ok:true,
    module:'RESTAURANT_V2_CASH_CLOSE_EMPTY_V80_DB',
