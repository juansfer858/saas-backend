...
function patchOperatorSource(source) {
  ...
  out = requiredReplace(
    ...
    "  const number = (v) => Number(v || 0);\n  const normalizeGlobalMenuSearch = (value) => String(value ?? '').normalize('NFD').replace(/[\\u0300-\\u036f]/g, '').toLocaleLowerCase('es').trim();\n  const globalMenuSearchTokens = (value) => normalizeGlobalMenuSearch(value).split(/\\s+/).filter(Boolean);\n  const globalMenuSearchMatches = (item, tokens) => { const haystack = normalizeGlobalMenuSearch([item?.product?.nombre, item?.product?.descripcion, item?.product?.codigo, item?.category, item?.station].filter(Boolean).join(' ')); return tokens.every((token) => haystack.includes(token)); };",
    'OP_HELPERS'
  );
  ...
  out = requiredReplace(
    ...
    "    const searchTokens = globalMenuSearchTokens(S.waiterSearch);\n    const category = S.waiterCategory || 'ENTRADAS';\n    const visibleMenu = S.menu.filter((item) => searchTokens.length ? globalMenuSearchMatches(item, searchTokens) : item.category === category);",
    'OP_VISIBLE_MENU'
  );
...
function patchDedicatedWaiterSource(source) {
  ...
  out = requiredReplace(
    ...
    "  function visibleMenuRows() {\n    const tokens = globalMenuSearchTokens(S.search);\n    return S.menu.filter((item) => tokens.length ? globalMenuSearchMatches(item, tokens) : item.category === S.category);\n  }",
    'WAITER_VISIBLE_MENU'
  );
