...
const MARKER = 'VANTIX_RESTAURANT_PRINT_TEMPLATE_EDITOR_V1';
...
const browserRuntime = String.raw`(() => {
  ...
  const canManage = ['ADMIN', 'SUPER_ADMIN'].includes(String(session.user?.rol || '').toUpperCase());
  if (!canManage) return;
  ...
  function ensureStyles() {
    ...
    style.textContent = \`
      ...
      .rpt-body{display:grid;grid-template-columns:minmax(0,1.05fr) minmax(300px,.95fr);gap:16px;padding:18px}.rpt-card{border:1px solid #e2e8f0;border-radius:14px;padding:14px;background:#fff}.rpt-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.rpt-field{display:grid;gap:5px;font-size:12px;font-weight:800;color:#334155}.rpt-field select,.rpt-field input[type=number]{min-height:40px;border:1px solid #cbd5e1;border-radius:9px;padding:0 10px;background:#fff;color:#0f172a}.rpt-checks{display:grid;gap:8px;margin-top:12px}.rpt-check{display:flex;align-items:center;gap:9px;font-size:12px;font-weight:750;color:#334155}.rpt-check input{width:18px;height:18px}
      ...
      .rpt-preview-head{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:10px}.rpt-preview-head b{font-size:13px}.rpt-preview-head select{min-height:36px;border:1px solid #cbd5e1;border-radius:8px;background:#fff;padding:0 9px}.rpt-paper{margin:0 auto;padding:16px 10px;background:#fffdf7;border:1px solid #cbd5e1;box-shadow:0 7px 20px rgba(15,23,42,.08);font:700 13px/1.38 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;white-space:pre;color:#111827;overflow:auto}.rpt-paper[data-paper="58"]{width:min(270px,100%)}.rpt-paper[data-paper="80"]{width:min(360px,100%)}.rpt-note{margin-top:10px;color:#64748b;font-size:11px;line-height:1.45}
  ...
  function alignText(value, width, align) {
    const text = String(value || '');
    if (align !== 'CENTER' || text.length >= width) return text;
  ...
  function fillForm(panel, config) {
    ...
    set('#rptItemAlign', config.itemAlign);
    set('#rptNoteAlign', config.noteAlign);
    set('#rptSeatAlign', config.seatAlign);
    set('#rptHeaderSize', config.headerSize);
    set('#rptItemSize', config.itemSize);
    set('#rptNoteSize', config.noteSize);
    set('#rptSeparator', config.separatorStyle);
    set('#rptSpacing', config.blankLinesBetweenItems);
  ...
  function ensureOverlay() {
    ...
    overlay.innerHTML = \`<section class="rpt-panel" role="dialog" aria-modal="true" aria-labelledby="rptTitle">
      ...
      <div class="rpt-body">
        <section class="rpt-card">
          ...
          <div class="rpt-actions"><button id="rptSave" class="rpt-btn primary" type="button">Guardar plantilla</button><button id="rptReset" class="rpt-btn secondary" type="button">Restaurar diseño recomendado</button></div>
          <div id="rptMessage" class="rpt-message"></div>
        </section>
        <aside class="rpt-card"><div class="rpt-preview-head"><b>Vista previa térmica</b><select id="rptPaperSize"><option value="80">80 mm</option><option value="58">58 mm</option></select></div><pre id="rptPreview" class="rpt-paper" data-paper="80"></pre><div id="rptSource" class="rpt-note"></div><div class="rpt-note">El ancho real lo toma cada impresora configurada. Esta pantalla modifica estructura, alineación y visibilidad de la comanda.</div></aside>
      </div>
    </section>\`;
    ...
    $('#rptSave', overlay).addEventListener('click', async () => {
      const button = $('#rptSave', overlay);
      const message = $('#rptMessage', overlay);
      button.disabled = true;
      message.innerHTML = '<span>Guardando…</span>';
      try {
        const saved = await api('/api/v1/restaurante/plantilla-impresion', { method:'PUT', body:JSON.stringify(formValue(overlay)) });
        fillForm(overlay, saved);
        renderPreview(overlay);
        $('#rptSource', overlay).textContent = 'Plantilla propia de este restaurante · se aplicará a las próximas comandas.';
        message.innerHTML = '<span class="rpt-ok">✓ Plantilla guardada.</span>';
      } catch (error) {
        message.innerHTML = \`<span class="rpt-error">\${esc(error.message)}</span>\`;
      } finally { button.disabled = false; }
    });
    $('#rptReset', overlay).addEventListener('click', async () => {
      const button = $('#rptReset', overlay);
      const message = $('#rptMessage', overlay);
      button.disabled = true;
      try {
        const restored = await api('/api/v1/restaurante/plantilla-impresion/restaurar', { method:'POST', body:JSON.stringify({}) });
        fillForm(overlay, restored);
        renderPreview(overlay);
        $('#rptSource', overlay).textContent = 'Diseño recomendado de VantixGC.';
        message.innerHTML = '<span class="rpt-ok">✓ Diseño recomendado restaurado.</span>';
      } catch (error) {
        message.innerHTML = \`<span class="rpt-error">\${esc(error.message)}</span>\`;
      } finally { button.disabled = false; }
    });
  ...
  async function openEditor() {
    ...
    const message = $('#rptMessage', overlay);
    message.textContent = 'Cargando plantilla…';
    try {
      const config = await api('/api/v1/restaurante/plantilla-impresion');
      fillForm(overlay, config);
      renderPreview(overlay);
      $('#rptSource', overlay).textContent = config.source === 'TENANT_OVERRIDE' ? 'Plantilla propia de este restaurante.' : 'Diseño recomendado de VantixGC.';
      message.textContent = '';
    } catch (error) {
      message.innerHTML = \`<span class="rpt-error">\${esc(error.message)}</span>\`;
    }
  ...
  function injectAction() {
    ...
    button.dataset.ccPrintTemplate = 'true';
    button.innerHTML = '🧾 Plantillas de impresión';
...
function installPrintTemplateEditorRuntime(req, res, next) {
  ...
  res.send = (body) => {
    ...
    res.set('X-VantixGC-Print-Template-Editor', 'v1');
