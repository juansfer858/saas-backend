...
const MARKER = 'VANTIX_RESTAURANT_PRINT_TEMPLATE_EDITOR_V4';
...
function browserInstaller() {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .rpt-body{display:grid;grid-template-columns:minmax(0,1.05fr) minmax(300px,.95fr);gap:16px;padding:18px}.rpt-card{border:1px solid #e2e8f0;border-radius:14px;padding:14px;background:#fff}.rpt-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.rpt-field{display:grid;gap:5px;font-size:12px;font-weight:800;color:#334155}.rpt-field select,.rpt-field input{min-height:40px;border:1px solid #cbd5e1;border-radius:9px;padding:0 10px;background:#fff;color:#0f172a}.rpt-field.full{grid-column:1/-1}.rpt-checks{display:grid;gap:8px;margin-top:12px}.rpt-check{display:flex;align-items:center;gap:9px;font-size:12px;font-weight:750;color:#334155}.rpt-check input{width:18px;height:18px}
      ...
      @media(max-width:420px){.rpt-grid{grid-template-columns:1fr}.rpt-field.full{grid-column:auto}.rpt-body{gap:10px}.rpt-card{padding:11px}}
    `;
  ...
  function previewText(config, paper) {
    ...
    if (config.customHeaderText) {
      lines.push(alignText(String(config.customHeaderText).toUpperCase(), width, config.itemAlign));
      lines.push('');
    }
    ...
    if (config.customFooterText) {
      lines.push('');
      lines.push(alignText(String(config.customFooterText).toUpperCase(), width, config.itemAlign));
    }
  ...
  function formValue(panel) {
    return {
      ...
      customHeaderText: $('#rptCustomHeader', panel).value,
      customFooterText: $('#rptCustomFooter', panel).value,
  ...
  function fillForm(panel, config) {
    ...
    set('#rptCustomHeader', config.customHeaderText); set('#rptCustomFooter', config.customFooterText);
  ...
  function ensureOverlay() {
    ...
    overlay.innerHTML = `<section class="rpt-panel" role="dialog" aria-modal="true" aria-labelledby="rptTitle">
      <header class="rpt-head"><div><h2 id="rptTitle">Plantilla de comanda</h2><p>Cocina · Barra · Postres. Caja, recibos y documentos fiscales no cambian aquí.</p></div><button class="rpt-close" type="button" data-rpt-close aria-label="Cerrar">×</button></header>
      <div class="rpt-body"><section class="rpt-card"><div class="rpt-grid">
        ...
        <label class="rpt-field full">Texto adicional arriba (opcional)<input id="rptCustomHeader" maxlength="48" placeholder="Ej. PRIORIDAD COCINA"></label>
        <label class="rpt-field full">Texto adicional abajo (opcional)<input id="rptCustomFooter" maxlength="48" placeholder="Ej. VERIFICAR ANTES DE ENTREGAR"></label>
      ...
      </div><div class="rpt-note">El texto adicional usa el mismo tamaño y alineación configurados para los productos. En domicilios se agregan automáticamente NOMBRE DEL CLIENTE, TELÉFONO y DIRECCIÓN.</div><div class="rpt-actions"><button id="rptSave" class="rpt-btn primary" type="button">Guardar plantilla</button><button id="rptReset" class="rpt-btn secondary" type="button">Restaurar diseño recomendado</button></div><div id="rptMessage" class="rpt-message"></div></section>
      <aside class="rpt-card"><div class="rpt-preview-head"><b>Vista previa térmica</b><select id="rptPaperSize"><option value="80">80 mm</option><option value="58">58 mm</option></select></div><pre id="rptPreview" class="rpt-paper" data-paper="80"></pre><div id="rptSource" class="rpt-note"></div><div class="rpt-note">La vista previa usa 48 columnas para 80 mm y 32 columnas para 58 mm. Las categorías comerciales no se imprimen en la comanda.</div></aside></div>
    </section>`;
  ...
  function renameStationTemplateButton() {
    const button = document.querySelector('[data-rkds-print-template]');
    if (!button) return;
  ...
  document.addEventListener('click', (event) => {
    if (event.target?.closest?.('[data-rkds-manage]')) requestAnimationFrame(renameStationTemplateButton);
  }, true);
  renameStationTemplateButton();
...
function installPrintTemplateEditorRuntime(req, res, next) {
  ...
  res.send = (body) => {
    ...
    res.set('X-VantixGC-Print-Template-Editor', 'v4-command-template');
