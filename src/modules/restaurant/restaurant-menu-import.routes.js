...
const menuExport = require('./restaurant-menu-export.service');
...
router.get('/carta-importacion/exportar',requirePermission('RESTAURANTE.ADMINISTRAR'),async(req,res,next)=>{try{const result=await menuExport.exportCarta(req.tenantId);res.setHeader('Content-Type','application/vnd.ms-excel; charset=utf-8');res.setHeader('Content-Disposition',`attachment; filename="${result.filename}"`);res.setHeader('Cache-Control','no-store');res.setHeader('X-VantixGC-Restaurant-Menu-Export','v130');res.send(result.buffer);}catch(error){next(error);}});
