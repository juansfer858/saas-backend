...
;(()=>{
  ...
  function ensureCloseDownload(){if(!allowed())return;const panel=$('.cash-close-panel');if(!panel||$('#dailySalesV116',panel))return;const anchor=$('#closeShift',panel)||panel.querySelector('button');const b=document.createElement('button');b.id='dailySalesV116';b.type='button';b.className='ri-btn small daily-sales-v116';b.textContent='Descargar ventas del día';b.onclick=()=>downloadSales(businessDate());if(anchor?.parentElement)anchor.parentElement.appendChild(b);else panel.appendChild(b)}
