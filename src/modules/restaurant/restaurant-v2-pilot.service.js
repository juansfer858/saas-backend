...
const SURFACES = Object.freeze({
  ...
  waiter: Object.freeze({ label: 'Mesero V2', url: '/app/centro-de-control/mesero-v2/' }),
  production: Object.freeze({ label: 'Producción V2', url: '/app/produccion-v2/' }),
...
async function readiness(tenantId, client = prisma) {
  const [tenant, configRow, admins, tables, menuItems, waiters, productionUsers, cashAccounts, manualStations] = await Promise.all([
    ...
    client.cajaBanco.count({ where: { tenantId, activo: true } }),
    client.restaurantProductionStation?.count
      ? client.restaurantProductionStation.count({ where: { tenantId, active: true } })
      : Promise.resolve(0)
  ]);
  ...
  const optional = [
    { key: 'waiters', label: 'Meseros activos', ok: waiters > 0, value: waiters, gate: false },
    { key: 'productionUsers', label: 'Personal Cocina/Barra/Postres', ok: productionUsers > 0, value: productionUsers, gate: false },
    { key: 'cashAccounts', label: 'Cajas/Bancos activos', ok: cashAccounts > 0, value: cashAccounts, gate: false },
  ...
  return {
    ...
    blockers,
    warnings,
    notGates: { dian: true, push: true, manualStations: true },
    rule: 'DIAN_PUSH_Y_ESTACIONES_MANUALES_NO_BLOQUEAN_OPERACION'
  };
}
...
async function setPilot(tenantId, userId, input = {}, client = prisma) {
  ...
  const checks = await readiness(tenantId, client);
  if (input.enabled && !checks.ready) {
    throw new AppError(409, `El piloto no está listo: ${checks.blockers.join(', ')}`, 'RESTAURANT_V2_PILOT_NOT_READY');
  }
  ...
  return client.$transaction(async (tx) => {
    ...
    const nextThemeData = { ...themeData, [PILOT_KEY]: nextPilot };
    await tx.restaurantConfig.update({ where: { tenantId }, data: { themeData: nextThemeData } });
    ...
    await tx.auditoriaContable.create({
      data: {
        ...
        entidadId: tenantId,
        accion: 'UPDATE',
        metadata: { before, after, readiness: { ready: checks.ready, blockers: checks.blockers, warnings: checks.warnings }, marker: MARKER }
      }
    });
