...
function withOperationalService(data){
  ...
  return {
    ...
    service:{
      ...data.service,
      operationalItems:groupOperationalLines(data.service.allItems||[],{includeState:true,includeSource:true})
    }
  };
}
...
router.patch('/v2/sesiones/:sessionId/items/:itemId',requirePermission('PEDIDOS.CREAR'),async(req,res,next)=>{try{res.json({ok:true,data:withOperationalService(await identity.updateOrderItemMeta(req.tenantId,req.user,req.params.sessionId,req.params.itemId,parse(metaSchema,req.body),V2_OPTIONS))})}catch(error){next(error)}});
router.patch('/v2/sesiones/:sessionId/personas',requirePermission('PEDIDOS.CREAR'),async(req,res,next)=>{try{const input=parse(peopleSchema,req.body);res.json({ok:true,data:withOperationalService(await identity.updateTableServiceSetup(req.tenantId,req.user,req.params.sessionId,{guestCount:input.guestCount},V2_OPTIONS))})}catch(error){next(error)}});
...
module.exports={restaurantV2OrdersRouter:router,V2_OPTIONS,listTablesV2,withOperationalService};
