...
async function createPairing(tenantId, createdByUserId, input) {
  ...
  const row = await prisma.trackingLink.create({ data: { tenantId, tokenHash: hashToken(rawToken), tokenCiphertext: encryptJson({ purpose: 'PAIRING', waiterUserId: waiter.id, createdAt: now.toISOString() }), tokenHint: rawToken.slice(-6), originType: ORIGIN_TYPE, originId, publicReference: waiter.id, currentStatus: 'PAIRING', timeline: [{ type: 'PAIRING_CREATED', at: now.toISOString(), waiterUserId: waiter.id, deviceName }], expiresAt, active: true, createdByUserId } });
...
async function inspectPairing(rawToken) {
  ...
  const now = new Date();
  if (!row || row.originType !== ORIGIN_TYPE || !row.active || row.currentStatus !== 'PAIRING' || row.expiresAt <= now) throw new AppError(410, 'Este vínculo ya venció o fue utilizado', 'RESTAURANT_WAITER_PAIRING_EXPIRED');
  const [waiter, tenant] = await Promise.all([assertMesero(row.tenantId, row.publicReference), prisma.tenant.findUnique({ where: { id: row.tenantId }, select: { id: true, nombreEmpresa: true, subdomain: true, moneda: true } })]);
...
async function claimPairing(rawToken, input = {}) {
  ...
  const token = permanentSessionToken(result.waiter, result.row.id);
  return { deviceId: result.row.id, deviceName: result.deviceName, persistent: true, activatedUntil: null, session: { token, subdomain: result.tenant.subdomain, tenant: result.tenant, user: result.waiter, persistent: true } };
}
...
async function renewDeviceSession(deviceId, tenantId, userId) {
  ...
  return { deviceId: row.id, persistent: true, session: { token, subdomain: tenant.subdomain, tenant, user: waiter, persistent: true } };
}
...
module.exports = { ORIGIN_TYPE, PAIRING_TTL_MS, DEVICE_PERSISTENT_UNTIL, LAST_SEEN_WRITE_INTERVAL_MS, createPairing, inspectPairing, claimPairing, renewDeviceSession, listDevices, revokeDevice, assertActiveDevice };
