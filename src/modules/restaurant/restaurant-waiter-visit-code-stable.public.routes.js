...
;(()=>{
  ...
  function renderStatus(tableId,status){
    ...
    card.dataset.signature=nextSignature;card.dataset.loaded='1';
    if(!status?.open){card.innerHTML='<div><small>AUTOPEDIDO QR</small><b>Mesa cerrada</b><span>El código aparecerá automáticamente cuando abras la mesa.</span></div>';return true;}
  ...
  function scheduleBurst(forceFirst=false){
    ...
    [0,120,320,700,1400].forEach((delay,index)=>setTimeout(()=>{if(token===burstToken) refreshCode(forceFirst&&index===0).catch(()=>{});},delay));
  }
  ...
  document.addEventListener('click',(event)=>{
    const table=event.target.closest?.('[data-waiter-table]');
    const open=event.target.closest?.('#waiterOpenTable');
    const tab=event.target.closest?.('[data-tab="mesero"]');
    if(table||open||tab) scheduleBurst(true);
  },true);
  window.addEventListener('vantix:tenant-realtime',(event)=>{if(realtimeRelevant(event.detail)) refreshCode(true).catch(()=>{});});
  window.addEventListener('vantix:tenant-realtime-ready',()=>refreshCode(false).catch(()=>{}));
...
function installWaiterVisitCodeStableRuntime(req, res, next) {
...
module.exports = { MARKER, runtime, installWaiterVisitCodeStableRuntime };
