...
const PANEL_TYPOGRAPHY = Object.freeze({ display: PANEL_FONT, body: PANEL_FONT, mono: PANEL_FONT });
const MAX_SPOTLIGHT_IMAGE_DATA_URL = 700000;
...
function cleanImageDataUrl(value) {
  ...
  if (text.length > MAX_SPOTLIGHT_IMAGE_DATA_URL || !IMAGE_DATA_URL_RE.test(text)) throw new AppError(400, 'La imagen de la promo no es válida o es demasiado grande', 'RESTAURANT_SPOTLIGHT_IMAGE_INVALID');
  return text;
}
function normalizeSpotlight(value) {
  ...
  try { imageDataUrl = cleanImageDataUrl(stored.imageDataUrl); } catch { imageDataUrl = null; }
  return { active:Boolean(stored.active && stored.menuItemId), kind, menuItemId:stored.menuItemId || null, label:String(stored.label || fallbackLabel).trim().slice(0,60), description:stored.description ? String(stored.description).trim().slice(0,180) : null, imageDataUrl };
}
...
async function validateSpotlight(tenantId, value) {
  const spotlight = normalizeSpotlight({ ...value, imageDataUrl:cleanImageDataUrl(value?.imageDataUrl) });
  ...
  const menu = await prisma.restaurantMenuItem.findFirst({where:{id:spotlight.menuItemId,tenantId,active:true}});
...
async function saveTheme(tenantId,userId,input) {
  ...
  let nextSpotlight = before.clientSpotlight;
  if (hasSpotlight) {
    const raw = {...(input.clientSpotlight || {})};
    if (!Object.prototype.hasOwnProperty.call(raw,'imageDataUrl')) raw.imageDataUrl = before.clientSpotlight.imageDataUrl || null;
    nextSpotlight = await validateSpotlight(tenantId,raw);
  }
  const nextData = {...stored,preset:input.preset || before.preset,restaurantName:Object.prototype.hasOwnProperty.call(input,'restaurantName') ? input.restaurantName || null : before.restaurantName,tokens:{...before.tokens,...(input.tokens||{})},typography:{...PANEL_TYPOGRAPHY},clientSpotlight:nextSpotlight};
...
module.exports={PANEL_FONT,PANEL_TYPOGRAPHY,DEFAULT_THEME,DEFAULT_SPOTLIGHT,MAX_SPOTLIGHT_IMAGE_DATA_URL,IMAGE_DATA_URL_RE,cleanImageDataUrl,publicTheme,getTheme,saveTheme};
