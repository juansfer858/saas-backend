...
tenantRouter.get('/installations', async (req, res, next) => {
  try { res.json({ ok: true, data: await platform.listInstallations(req.tenantId) }); }
  catch (error) { next(error); }
});

tenantRouter.get('/releases', async (req, res, next) => {
  try { res.json({ ok: true, data: await platform.listReleases(req.tenantId) }); }
  catch (error) { next(error); }
});

tenantRouter.post('/releases', async (req, res, next) => {
  try { res.status(201).json({ ok: true, data: await platform.createRelease(req.tenantId, req.userId, parse(releaseSchema, req.body)) }); }
  catch (error) { next(error); }
});
...
tenantRouter.get('/relay/requests/:id', async (req, res, next) => {
  try { res.json({ ok: true, data: await platform.getRelayRequest(req.tenantId, req.params.id) }); }
  catch (error) { next(error); }
});

tenantRouter.get('/remote-channels', async (req, res, next) => {
  try { res.json({ ok: true, data: await platform.listRemoteChannels(req.tenantId) }); }
  catch (error) { next(error); }
});
...
tenantRouter.post('/remote-channels', async (req, res, next) => {
  try {
    ...
    res.status(201).json({ ok: true, data: await platform.createRemoteChannel(req.tenantId, input.edgeAgentId, input) });
  } catch (error) { next(error); }
});

tenantRouter.post('/remote-channels/:id/rotate-token', async (req, res, next) => {
  try { res.json({ ok: true, data: await platform.rotateRemoteChannel(req.tenantId, req.params.id) }); }
  catch (error) { next(error); }
});

tenantRouter.get('/remote-orders', async (req, res, next) => {
  try { res.json({ ok: true, data: await platform.listRemoteOrders(req.tenantId, req.query) }); }
  catch (error) { next(error); }
});

tenantRouter.post('/remote-orders/:id/approve', async (req, res, next) => {
  try { res.json({ ok: true, data: await platform.decideRemoteOrder(req.tenantId, req.params.id, 'APPROVE') }); }
  catch (error) { next(error); }
});

tenantRouter.post('/remote-orders/:id/reject', async (req, res, next) => {
  try { res.json({ ok: true, data: await platform.decideRemoteOrder(req.tenantId, req.params.id, 'REJECT') }); }
  catch (error) { next(error); }
});
...
publicRouter.post('/heartbeat', async (req, res, next) => {
  try { res.json({ ok: true, data: await platform.heartbeat(req.edgeAgent, parse(heartbeatSchema, req.body)) }); }
  catch (error) { next(error); }
});

publicRouter.get('/update/manifest', async (req, res, next) => {
  try { res.json({ ok: true, data: await platform.updateManifest(req.edgeAgent) }); }
  catch (error) { next(error); }
});

publicRouter.post('/update/report', async (req, res, next) => {
  try { res.json({ ok: true, data: await platform.reportDeployment(req.edgeAgent, parse(deploymentReportSchema, req.body)) }); }
  catch (error) { next(error); }
});

publicRouter.get('/relay/pull', async (req, res, next) => {
  try { res.json({ ok: true, data: await platform.pullRelayRequests(req.edgeAgent, req.query.limit) }); }
  catch (error) { next(error); }
});

publicRouter.post('/relay/:id/complete', async (req, res, next) => {
  try { res.json({ ok: true, data: await platform.completeRelayRequest(req.edgeAgent, req.params.id, parse(relayCompleteSchema, req.body || {})) }); }
  catch (error) { next(error); }
});

publicRouter.get('/remote-orders/pull', async (req, res, next) => {
  try { res.json({ ok: true, data: await platform.pullRemoteOrders(req.edgeAgent, req.query.limit) }); }
  catch (error) { next(error); }
});

publicRouter.post('/remote-orders/:id/report', async (req, res, next) => {
  try { res.json({ ok: true, data: await platform.reportRemoteOrder(req.edgeAgent, req.params.id, parse(remoteReportSchema, req.body || {})) }); }
  catch (error) { next(error); }
});
