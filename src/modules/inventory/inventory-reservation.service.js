...
async function reservedQuantity(client, tenantId, productId, options = {}) {
  ...
  const result = await client.inventoryReservation.aggregate({
    where,
    _sum: { quantity: true }
  });
...
async function availability(tenantId, productId, client = prisma) {
  ...
  return {
    ...
    physical,
    reserved,
    available: physical.minus(reserved).toDecimalPlaces(4)
  };
}
...
async function reserveInTx(tx, params) {
  ...
  if (product.tipo !== 'PRODUCTO' || !product.controlaInventario) {
    throw new AppError(409, 'El producto no admite reserva de inventario', 'INVENTORY_RESERVATION_NOT_APPLICABLE');
  }

  const key = {
    tenantId_sourceType_sourceLineId: {
  ...
  if (existing?.state === 'ACTIVE') {
    if (existing.productId !== params.productId || qty(existing.quantity).neq(quantity)) {
      throw new AppError(409, 'La línea ya tiene una reserva diferente', 'INVENTORY_RESERVATION_CONFLICT');
    }
    return existing;
  }
...
async function releaseSourceLineInTx(tx, tenantId, sourceType, sourceLineId, state = 'RELEASED') {
  const current = await tx.inventoryReservation.findUnique({
    where: { tenantId_sourceType_sourceLineId: { tenantId, sourceType, sourceLineId } }
  });
...
async function markSourceConsumedInTx(tx, tenantId, sourceType, sourceId) {
  const rows = await tx.inventoryReservation.findMany({
    where: { tenantId, sourceType, sourceId, state: { in: ['ACTIVE', 'RELEASED'] } }
  });
  if (!rows.length) return [];
  const now = new Date();
  await tx.inventoryReservation.updateMany({
    where: { id: { in: rows.map((row) => row.id) } },
    data: { state: 'CONSUMED', consumedAt: now, releasedAt: now }
  });
  return rows;
}
...
module.exports = {
  ...
  reserve,
  reserveInTx,
  releaseSourceLineInTx,
  releaseSourceInTx,
  markSourceConsumedInTx,
  listReservations
};
