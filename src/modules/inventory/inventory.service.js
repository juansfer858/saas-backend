...
async function createNegativeStockAlert(tx, context, product, oldStock, quantity, newStock, params) {
  ...
  await tx.edgeReconciliationAlert.upsert({
    where: { edgeAgentId_operationId_type_productoId: { edgeAgentId: context.edgeAgentId, operationId: context.operationId, type: 'NEGATIVE_STOCK', productoId: product.id } },
    create: {
      ...
      details: { sku: product.sku, stockCentralAntes: oldStock.toString(), cantidadSincronizada: quantity.toString(), stockCentralDespues: newStock.toString(), referencia: params.referencia || null }
    },
    update: {
      state: 'OPEN', severity: 'HIGH', originDocumentId: params.comprobanteId || null,
      message: `Stock negativo al reconciliar operación offline: ${product.nombre}`,
      details: { sku: product.sku, stockCentralAntes: oldStock.toString(), cantidadSincronizada: quantity.toString(), stockCentralDespues: newStock.toString(), referencia: params.referencia || null }
    }
  });
...
async function applyMovement(tx, params) {
  ...
  if (newStock.lt(0) && edgeContext?.allowNegativeInventory) await createNegativeStockAlert(tx, edgeContext, product, oldStock, quantity, newStock, params);
