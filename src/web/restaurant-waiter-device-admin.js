(() => {
  ...
  async function openManager(preselectedUserId = null) {
    ...
    try {
      ...
      dialog.innerHTML = `<div style="padding:20px 22px;border-bottom:1px solid #e2e8f0;display:flex;justify-content:space-between;gap:12px;align-items:flex-start"><div><div class="ri-eyebrow">Dispositivos de Mesero</div><h2 style="margin:3px 0">Conectar tablets o celulares</h2><p class="ri-muted" style="margin:4px 0 0">Cada mesero tiene un QR reutilizable. El mismo QR puede conectar varios equipos y cada uno se administra por separado.</p></div><button type="button" class="ri-btn" data-wd-close>Cerrar</button></div>
        <div style="padding:20px 22px;display:grid;gap:18px">
          <section class="ri-card" style="padding:16px!important;box-shadow:none!important"><div style="display:grid;grid-template-columns:minmax(220px,1fr) minmax(180px,.8fr) auto;gap:10px;align-items:end">
            ...
            <label class="ri-label">Nombre sugerido<input id="wdDeviceName" class="ri-input" maxlength="80" value="Tablet Mesero"></label>
            <button type="button" class="ri-btn primary" id="wdGenerate" ${waiters.length ? '' : 'disabled'}>Ver QR</button>
      ...
      dialog.querySelector('#wdGenerate')?.addEventListener('click', async () => {
        ...
        if (!userId) { result.innerHTML = '<div class="ri-error" style="margin-top:12px">Selecciona el mesero.</div>'; return; }
  ...
  window.VantixGCWaiterDeviceAdmin = Object.freeze({ open:(userId=null)=>openManager(userId), reusableQr:true, multipleDevicesPerWaiter:true });
})();
