(() => {
  ...
  function inventoryPurchaseLine(index, products, preset = {}) {
  ...
  window.openInventoryPurchase = async function openInventoryPurchase() {
    try {
      ...
      const products = (productResponse.data || []).filter((p) => p.activo !== false && p.tipo !== 'SERVICIO' && p.controlaInventario === true);
      ...
      const recalc = () => {
        ...
        [...lines.querySelectorAll('[data-purchase-line]')].forEach((row) => {
          ...
          const base = qty * cost, iva = base * vat / 100;
          subtotal += base; tax += iva;
          row.querySelector('[data-purchase-line-total]').textContent = money(base + iva);
        });
        $('#invPurSubtotal').textContent = money(subtotal);
        $('#invPurTax').textContent = money(tax);
        $('#invPurTotal').textContent = money(subtotal + tax);
      };
      const addLine = (preset = {}) => {
        lines.insertAdjacentHTML('beforeend', inventoryPurchaseLine(lineIndex++, products, preset));
        recalc();
      };
      const payload = () => ({
        proveedorId: $('#invPurSupplier').value,
        fecha: $('#invPurDate').value,
        referenciaExterna: $('#invPurReference').value.trim(),
        condicionPagoDias: Number($('#invPurTerms').value || 0),
        notas: $('#invPurNotes').value.trim() || null,
        detalles: [...lines.querySelectorAll('[data-purchase-line]')].map((row) => ({
      ...
      const save = async (emitNow) => {
        const errorBox = $('#inventoryPurchaseError');
        if (errorBox) errorBox.innerHTML = '';
        const buttons = [$('#invPurDraft'), $('#invPurReceive')].filter(Boolean);
        buttons.forEach((button) => { button.disabled = true; });
        try {
          const data = payload();
          if (!data.proveedorId) throw new Error('Seleccione un proveedor.');
      ...
      lines.addEventListener('change', (event) => {
        ...
        if (event.target.matches('[data-purchase-product]')) {
          ...
          if (product) row.querySelector('[data-purchase-vat]').value = Number(product.ivaPct || 0);
        }
        recalc();
      });
      lines.addEventListener('click', (event) => {
        ...
        if (!button) return;
        if (lines.querySelectorAll('[data-purchase-line]').length <= 1) return;
        button.closest('[data-purchase-line]')?.remove();
        recalc();
      });
      $('#invPurAddLine').onclick = () => addLine();
      $('#invPurCancel').onclick = () => modal.remove();
      $('#invPurDraft').onclick = () => save(false);
      $('#invPurReceive').onclick = () => save(true);
  ...
  async function inventoryView() {
    ...
    const productTable = (list, title = 'Productos') => `<div class="panel"><div class="panel-head"><h2>${esc(title)}</h2><span class="muted">${list.length} registros</span></div><div class="table-wrap"><table class="table"><thead><tr><th>SKU</th><th>Producto</th><th class="money">Stock</th><th class="money">Costo promedio</th><th class="money">Precio</th><th>Historial</th></tr></thead><tbody>${list.map(x=>`<tr><td>${esc(x.sku)}</td><td>${esc(x.nombre)}</td><td class="money">${Number(x.stockActual||0).toFixed(4)}</td><td class="money">${money(x.costoPromedio)}</td><td class="money">${money(x.precio1)}</td><td><button class="btn small" type="button" onclick="openInventoryProductHistory('${esc(x.id)}')">Historial</button></td></tr>`).join('')}</tbody></table></div></div>`;
