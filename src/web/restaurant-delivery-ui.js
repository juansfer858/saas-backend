(() => {
  ...
  const demoFourCards = String(session.subdomain || '').trim().toLowerCase() === DEMO_TENANT;
  if (demoFourCards) document.documentElement.dataset.deliveryDemoGrid = '4';
  let poll = null;
  let active = false;
  let creating = false;
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      html[data-delivery-demo-grid="4"] .delivery-list{grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;align-items:start}
      html[data-delivery-demo-grid="4"] .delivery-card{min-width:0;padding:13px 14px 14px;gap:8px;border-radius:14px;box-shadow:0 4px 14px rgba(15,23,42,.06)}
      html[data-delivery-demo-grid="4"] .delivery-card-top{align-items:flex-start;gap:8px}
      html[data-delivery-demo-grid="4"] .delivery-code{font-size:15px;line-height:1.15;overflow-wrap:anywhere}
      html[data-delivery-demo-grid="4"] .delivery-age{font-size:10px}
      html[data-delivery-demo-grid="4"] .delivery-state{flex:0 0 auto;min-height:24px;padding:3px 7px;font-size:9px}
      html[data-delivery-demo-grid="4"] .delivery-card-customer{padding:9px 10px;border-radius:10px;background:#f8fafc}
      html[data-delivery-demo-grid="4"] .delivery-card-customer b{font-size:15px;line-height:1.2}
      html[data-delivery-demo-grid="4"] .delivery-card-customer span{font-size:11px;line-height:1.3;overflow-wrap:anywhere}
      html[data-delivery-demo-grid="4"] .delivery-items{gap:5px;padding:8px 0}
      html[data-delivery-demo-grid="4"] .delivery-item{font-size:11px;line-height:1.3;align-items:flex-start}
      html[data-delivery-demo-grid="4"] .delivery-item>span{min-width:0;overflow-wrap:anywhere}
      html[data-delivery-demo-grid="4"] .delivery-item>b{flex:0 0 auto;font-size:11px}
      html[data-delivery-demo-grid="4"] .delivery-total strong{font-size:22px}
      html[data-delivery-demo-grid="4"] .delivery-payment{font-size:9px}
      html[data-delivery-demo-grid="4"] .delivery-actions{grid-template-columns:minmax(0,1fr) auto}
      html[data-delivery-demo-grid="4"] .delivery-actions .primary-next{min-height:44px;padding:0 8px;font-size:11px}
      html[data-delivery-demo-grid="4"] .delivery-actions .secondary-next{min-height:44px;padding:0 10px;font-size:11px}
      @media(max-width:1300px){html[data-delivery-demo-grid="4"] .delivery-list{grid-template-columns:repeat(3,minmax(0,1fr))}}
      @media(max-width:1050px){html[data-delivery-demo-grid="4"] .delivery-list{grid-template-columns:repeat(2,minmax(0,1fr))}}
      @media(max-width:780px){html[data-delivery-demo-grid="4"] .delivery-list{grid-template-columns:1fr}}
  ...
  function card(row) {
    ...
    const itemLimit = demoFourCards ? 3 : 5;
    const itemRows = (row.items || []).slice(0, itemLimit);
    return `<article class="delivery-card ${esc(row.state)} ${isLate(row) ? 'late' : ''}" data-delivery-card="${esc(row.id)}">
      ...
      <div class="delivery-items">${itemRows.map((item) => `<div class="delivery-item"><span>${esc(item.quantity)}× ${esc(item.description)}${item.unitPrice !== undefined && item.unitPrice !== null ? ` · ${money(item.unitPrice)} c/u` : ''}${item.notes ? `<small class="ri-muted" style="display:block;margin-top:2px">${esc(item.notes)}</small>` : ''}</span><b>${money(item.lineTotal)}</b></div>`).join('')}${(row.items || []).length > itemLimit ? `<small class="ri-muted">+ ${(row.items || []).length - itemLimit} producto(s)</small>` : ''}${Number(row.deliveryFee || 0) > 0 ? `<div class="delivery-item"><span>Servicio domicilio</span><b>${money(row.deliveryFee)}</b></div>` : ''}</div>
