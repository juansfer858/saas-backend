(() => {
  ...
  function card(row) {
    ...
    return `<article class="delivery-card ${esc(row.state)} ${isLate(row) ? 'late' : ''}" data-delivery-card="${esc(row.id)}">
      ...
      <div class="delivery-items">${itemRows.map((item) => `<div class="delivery-item"><span>${esc(item.quantity)}× ${esc(item.description)}${item.unitPrice !== undefined && item.unitPrice !== null ? ` · ${money(item.unitPrice)} c/u` : ''}${item.notes ? `<small class="ri-muted" style="display:block;margin-top:2px">${esc(item.notes)}</small>` : ''}</span><b>${money(item.lineTotal)}</b></div>`).join('')}${(row.items || []).length > 5 ? `<small class="ri-muted">+ ${(row.items || []).length - 5} producto(s)</small>` : ''}${Number(row.deliveryFee || 0) > 0 ? `<div class="delivery-item"><span>Servicio domicilio</span><b>${money(row.deliveryFee)}</b></div>` : ''}</div>
