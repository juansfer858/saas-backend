(() => {
  ...
  window.hydrateAccountingConfig = async function hydrateAccountingConfig() {
    try {
      ...
      $('#accountingReadiness').innerHTML = `<div class="cards">${Object.entries(data.modules).map(([k,v])=>`<div class="card metric"><div class="label">${moduleLabels[k]||k}</div><div class="value" style="font-size:17px;color:${v.ready?'var(--green)':'var(--warn)'}">${v.ready?'LISTO':'FALTA CONFIGURAR'}</div><div class="hint">${v.missing.length?esc(v.missing.join(', ')):'Mapeos requeridos completos'}</div></div>`).join('')}</div>`;
      $('#accountingMappings').innerHTML = `<div class="panel"><div class="panel-head"><h2>Parametrización contable</h2><span class="muted">Los módulos nunca usan números PUC hardcodeados.</span></div><div class="table-wrap"><table class="table"><thead><tr><th>Módulo</th><th>Parámetro</th><th>Cuenta PUC</th><th>Estado</th><th></th></tr></thead><tbody>${data.mappings.map(m=>`<tr><td>${esc(m.module)}</td><td><strong>${esc(m.label)}</strong><br><span class="muted">${esc(m.clave)}</span></td><td><select class="select" id="map-${m.clave}"><option value="">Seleccione…</option>${optionRows(common.accounts,m.cuenta?.id,a=>`${a.codigo} · ${a.nombre} · ${a.naturaleza}`)}</select></td><td>${m.ready?'<span class="badge b-paid">Listo</span>':'<span class="badge b-partial">Pendiente</span>'}</td><td><button class="btn small" onclick="saveAccountingMap('${m.clave}')">Guardar</button></td></tr>`).join('')}</tbody></table></div></div>`;
  ...
  async function carteraView() {
    ...
    return `<div class="pagehead"><div><h1>Cartera</h1><p>CxC y CxP derivadas de documentos, pagos y los mismos auxiliares contables.</p></div><button class="btn primary" onclick="openBatchPayment()">Aplicar pago a seleccionadas</button></div><div class="cards"><div class="card metric"><div class="label">Saldo abierto</div><div class="value">${money(a.totales.TOTAL)}</div></div><div class="card metric"><div class="label">Vencido 0–30</div><div class="value">${money(a.totales['0_30'])}</div></div><div class="card metric"><div class="label">Vencido 31–90</div><div class="value">${money(Number(a.totales['31_60'])+Number(a.totales['61_90']))}</div></div><div class="card metric"><div class="label">+90 días</div><div class="value">${money(a.totales.MAS_90)}</div></div></div><div class="notice">Conciliación CxC: ${cxc.data.cuadra?'CUADRA':'DIFERENCIA '+money(cxc.data.diferencia)} · CxP: ${cxp.data.cuadra?'CUADRA':'DIFERENCIA '+money(cxp.data.diferencia)}</div><div class="panel"><div class="panel-head"><h2>Documentos abiertos</h2></div><div class="table-wrap"><table class="table"><thead><tr><th></th><th>Tipo</th><th>Tercero</th><th>Referencia</th><th>Vence</th><th class="money">Saldo</th><th>Estado</th></tr></thead><tbody>${rows.map(x=>`<tr><td><input type="checkbox" class="portfolio-check" data-doc="${x.comprobanteId}" data-third="${x.terceroId}" data-type="${x.tipo}" data-saldo="${x.saldo}"></td><td>${esc(x.tipo)}</td><td>${esc(x.tercero?.nombre||'')}</td><td>${esc(x.referencia||'')}</td><td>${date(x.fechaVencimiento)}</td><td class="money">${money(x.saldo)}</td><td>${esc(x.estado)}</td></tr>`).join('')}</tbody></table></div></div>`;
  }
