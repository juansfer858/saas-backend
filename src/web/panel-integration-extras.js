(() => {
  ...
  function installInventoryProductCreatorV70() {
    const MARKER = 'VANTIX_INVENTORY_PRODUCT_CREATE_V70_BASE';
    if (window[MARKER]) return;
    window[MARKER] = Object.freeze({ version:'70.1.0', independentOfIntegrationCore:true });

    function isInventoryPage() {
      return location.pathname === '/app/inventario';
    }

    function ensureInventoryProductButton() {
    ...
    window.openInventoryProductFormV70 = function openInventoryProductFormV70() {
      ...
      form.onsubmit = async (event) => {
        ...
        try {
          ...
          await fetchJson('/api/v1/inventario/productos', {
            method:'POST',
            body:JSON.stringify({
              tipo:productType,
          ...
          document.querySelector('#inventoryProductModal')?.remove();
          if (typeof window.render === 'function') await window.render();
          requestAnimationFrame(ensureInventoryProductButton);
        } catch (error) {
    ...
    const observer = new MutationObserver(() => requestAnimationFrame(ensureInventoryProductButton));
    observer.observe(document.documentElement, { childList:true, subtree:true });
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', ensureInventoryProductButton, { once:true });
