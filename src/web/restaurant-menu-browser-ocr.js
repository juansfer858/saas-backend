(() => {
  ...
  const LAYOUT_MARKER = 'VANTIX_MENU_OCR_LAYOUT_V3';
  ...
  function price(v) {
    ...
    if (kilo) { const num = Number(raw.replace(',', '.')); return Number.isFinite(num) && num > 0 ? Math.round(num * 1000) : 0; }
  ...
  function familyCategory(product) { const value = key(product); for (const [pattern, category] of familyRules) if (pattern.test(value)) return category; return null; }
  function extractUppercaseTitle(value) {
    const words = productText(value).split(/\s+/).filter(Boolean).slice(0, 12); if (words.length < 2) return '';
    const title = [];
    ...
    const out = productText(title.join(' ')); return title.length >= 2 && out.length >= 4 && out.length <= 80 ? out : '';
  }
  function productCandidate(value) {
    const raw = String(value || '').trim(); if (!raw) return '';
    const strong = raw.split(/(?:\s*(?:->|→|—{1,}|–{1,}|={2,}|-{3,}|\|)\s*|\.{3,})/)[0] || raw;
    let cleaned = productText(strong).replace(/^[A-Za-z0-9]{1,2}\s*[-:)>]+\s*/, '');
    const uppercase = extractUppercaseTitle(cleaned); cleaned = productText(uppercase || cleaned);
    return cleaned.replace(/\s+(?:con|incluye|acompañado|acompanado)$/i, '').trim();
  }
  function candidateScore(value) {
    const text = productText(value); if (!text || noise(text)) return -1000;
  ...
  function priceMatches(line) {
    ...
    const out = []; let match;
    while ((match = re.exec(line)) !== null) { const value = price(match[0]); if (value > 0) out.push({ value, index:match.index, end:match.index + match[0].length }); if (match.index === re.lastIndex) re.lastIndex += 1; }
    return out;
  }
  function parseMenuText(text, options = {}) {
    const lines = String(text || '').replace(/\r/g, '\n').split(/\n+/).map((line) => line.replace(/[\t]+/g, ' ').trim()).filter(Boolean);
    let category = 'Otros', pending = []; const rows = [], seen = new Set();
    const baseConfidence = Number.isFinite(Number(options.baseConfidence)) ? Number(options.baseConfidence) : null;
    for (const rawLine of lines) {
      const line = clean(rawLine, 320); if (noise(line)) continue; const matches = priceMatches(line);
      if (!matches.length) { const h = heading(line); if (h) { category = h; pending = []; } else if (/[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]/.test(line) && line.length <= 100) { pending.push(line); if (pending.length > 4) pending.shift(); } continue; }
      const name = chooseProduct(rawLine.slice(0, matches[0].index), pending); pending = []; if (!name || name.length < 2) continue;
      ...
      const dedupe = `${key(commercial)}|${key(name)}|${matches[0].value}`; if (seen.has(dedupe)) continue; seen.add(dedupe);
      let confidence = baseConfidence ?? (category === 'Otros' ? .68 : .84); if (family) confidence += .05; if (candidateScore(name) < 35) confidence -= .12;
  ...
  function menuOcrScore(text) {
    ...
    const categories = new Set(rows.map((row) => key(row.category))); let score = rows.length * 140 + categories.size * 24;
    for (const line of lines) { if (priceMatches(line).length) score += 18; if (heading(line)) score += 12; score -= (line.match(/[{}<>_=~^`\\]/g) || []).length * 4; }
  ...
  async function prepareImage(blob) {
    ...
    try {
      ...
      const side = Math.max(width || 1, height || 1); let scale = 1;
      if (side < 2800) scale = Math.min(2, 3000 / side); if (side * scale > 4300) scale = 4300 / side;
  ...
  function rotateClockwise(canvas) {
    ...
    const ctx = rotated.getContext('2d', { alpha:false }); ctx.fillStyle = '#fff'; ctx.fillRect(0,0,rotated.width,rotated.height); ctx.translate(rotated.width,0); ctx.rotate(Math.PI/2); ctx.drawImage(canvas,0,0); return rotated;
  }
  async function createOcrWorker(languages) { await loadScript(TESSERACT_URL, () => Boolean(window.Tesseract?.createWorker)); return window.Tesseract.createWorker(languages); }

  async function recognizeMultiPass(source, profiles = LANGUAGE_PROFILES) {
    let best = null, passes = 0;
    for (const languages of profiles) {
      ...
      try {
        for (const psm of PSM_MODES) {
          ...
          const text = String(result?.data?.text || ''), confidence = Math.max(0, Math.min(100, Number(result?.data?.confidence || 0))), score = menuOcrScore(text) + confidence * 2;
          passes += 1;
          if (!best || score > best.score) best = { text, blocks:result?.data?.blocks || null, score, confidence, engine:`tesseract:${languages.join('+')}:psm${psm}`, passes };
        }
      } finally { await w.terminate().catch(() => {}); }
    }
    if (!best?.text?.trim()) throw new Error('El OCR no pudo reconocer texto útil en la carta.');
    best.passes = passes; return best;
  }

  async function detectVerticalCategories(canvas) {
    const layout = window.VantixGCRestaurantLayoutOcrV3;
    if (!layout?.detectCategoryAnchorsFromRotatedBlocks) return [];
    const rotated = rotateClockwise(canvas), w = await createOcrWorker(['spa','eng']);
    try {
      await w.setParameters({ tessedit_pageseg_mode:'11', preserve_interword_spaces:'1', user_defined_dpi:'220' });
      const result = await w.recognize(rotated, {}, { text:true, blocks:true });
      return layout.detectCategoryAnchorsFromRotatedBlocks(result?.data?.blocks || [], canvas.height);
    } finally { await w.terminate().catch(() => {}); }
  }

  function chooseLayoutRows(recognized, canvas, anchors) {
    const layout = window.VantixGCRestaurantLayoutOcrV3;
    const textRows = parseMenuText(recognized.text, { baseConfidence:Math.max(.5, Math.min(.98, Number(recognized.confidence || 0) / 100)) });
    if (!layout?.parseBlocks || !recognized.blocks) return { items:textRows, mode:'TEXT' };
    const layoutRows = layout.parseBlocks(recognized.blocks, canvas.width, canvas.height, anchors);
    if (layoutRows.length >= Math.max(3, Math.floor(textRows.length * .45))) return { items:layoutRows, mode:'LAYOUT', textRows:textRows.length, layoutRows:layoutRows.length };
    return { items:textRows, mode:'TEXT_FALLBACK', textRows:textRows.length, layoutRows:layoutRows.length };
  }

  async function ocrImage(blob) {
    const canvas = await prepareImage(blob), recognized = await recognizeMultiPass(canvas, LANGUAGE_PROFILES), anchors = await detectVerticalCategories(canvas).catch(() => []), parsed = chooseLayoutRows(recognized, canvas, anchors);
    return { ...recognized, ...parsed, anchors };
  }
  ...
  async function ocrPdf(blob) {
    ...
    const pdf = await window.pdfjsLib.getDocument({ data:new Uint8Array(await blob.arrayBuffer()) }).promise, pages = Math.min(Number(pdf.numPages || 0), MAX_PDF_PAGES);
    let textLayer = '';
    ...
    if (textRows.length >= 2 && menuOcrScore(textLayer) > 250) return { text:textLayer, items:textRows, mode:'PDF_TEXT', engine:'pdf:text-layer', confidence:97, score:menuOcrScore(textLayer), passes:0, anchors:[] };

    const allItems = []; let score=0, confidenceTotal=0, passTotal=0;
    for (let i=1;i<=pages;i+=1) {
      const page=await pdf.getPage(i), baseViewport=page.getViewport({scale:1}), targetScale=Math.min(3.1,4300/Math.max(baseViewport.width,baseViewport.height)), viewport=page.getViewport({scale:Math.max(1.8,targetScale)});
      ...
      const recognized=await recognizeMultiPass(canvas,[['spa','eng']]), anchors=await detectVerticalCategories(canvas).catch(()=>[]), parsed=chooseLayoutRows(recognized,canvas,anchors);
      allItems.push(...parsed.items); score+=recognized.score; confidenceTotal+=recognized.confidence; passTotal+=recognized.passes;
    }
    return { text:'', items:allItems, mode:'PDF_LAYOUT_OCR', engine:'pdf:ocr:layout-v3', confidence:pages?confidenceTotal/pages:0, score, passes:passTotal, anchors:[] };
  }
  ...
  async function browserAnalyze(payload) {
    const blob=base64Blob(payload), recognized=payload.mimeType==='application/pdf'?await ocrPdf(blob):await ocrImage(blob);
    const items=Array.isArray(recognized.items)?recognized.items:parseMenuText(recognized.text,{baseConfidence:Math.max(.5,Math.min(.98,Number(recognized.confidence||0)/100))});
    ...
    return { fileName:payload.fileName||'carta', mimeType:payload.mimeType, bytes:blob.size, provider:'BROWSER_OCR', marker:MARKER, qualityMarker:QUALITY_MARKER, layoutMarker:LAYOUT_MARKER, engine:recognized.engine, recognitionMode:recognized.mode||'TEXT', recognitionScore:Math.round(recognized.score||0), recognitionPasses:recognized.passes||0, categoryAnchors:Array.isArray(recognized.anchors)?recognized.anchors.length:0, items };
  }

  window.fetch = async (input, init = {}) => {
    const path=pathOf(input);
    if (path===STATUS_PATH) {
      try { const response=await nativeFetch(input,init), copy=response.clone(), body=await copy.json().catch(()=>null); if (response.ok&&body?.data?.configured) { browserMode=false; return response; } browserMode=true; } catch { browserMode=true; }
      return jsonResponse({ok:true,data:{configured:true,provider:'BROWSER_OCR',marker:MARKER,qualityMarker:QUALITY_MARKER,layoutMarker:LAYOUT_MARKER,maxBytes:MAX_BYTES,acceptedMimeTypes:['application/pdf','image/jpeg','image/png','image/webp'],preserveOriginalImage:true,multipass:true,layoutAware:true,verticalCategoryOcr:true,wordBoundingBoxes:true,languages:['spa+eng','spa','eng'],psmModes:PSM_MODES,note:'OCR multipase con geometría listo'}});
    }
    if (path===ANALYZE_PATH&&browserMode) {
      try { const payload=JSON.parse(String(init?.body||'{}')); return jsonResponse({ok:true,data:await browserAnalyze(payload)}); }
      catch(error) { return jsonResponse({ok:false,error:{code:'RESTAURANT_MENU_BROWSER_OCR_ERROR',message:error?.message||'No fue posible reconocer la carta'}},502); }
    }
    return nativeFetch(input,init);
  };

  window.VantixGCRestaurantBrowserOcr = { marker:MARKER, qualityMarker:QUALITY_MARKER, layoutMarker:LAYOUT_MARKER, parseMenuText, menuOcrScore, productCandidate };
})();
