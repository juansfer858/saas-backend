/* VANTIX_RESTAURANT_V2_DEVICE_SDK_P8 */
(()=>{'use strict';
  ...
  function readSession(){return readJson(sessionKey)}
  ...
  async function api(path,opts={}){
    ...
    if(response.status===401||response.status===403){
      ...
      window.dispatchEvent(new CustomEvent('vantix:restaurant-v2-device-revoked',{detail:{device,status:response.status}}));
      throw new Error(body?.error?.message||`La autorización de ${deviceLabel()} ya no es válida.`);
    }
  ...
  window.RestaurantV2=Object.freeze({marker:'VANTIX_RESTAURANT_V2_DEVICE_SDK_P8',version:'8.0.0',device,sessionKey,readSession,requireSession,api,can,esc,money,emit,on});
})();
