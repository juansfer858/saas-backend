...
function openMergeDialog(){
  ...
  body.innerHTML='<label>Nombre conjunto<input class="rv2-input" data-merge-name value="'+esc(accountLabel(dest))+'" maxlength="160"></label>'+[...groups.entries()].map(([tableName,rows])=>'<fieldset><legend>'+esc(tableName)+'</legend>'+rows.map(acc=>'<label style="display:flex;align-items:center;gap:8px;margin:6px 0"><input type="checkbox" data-merge-source="'+esc(acc.id)+'"><span style="flex:1">'+esc(accountLabel(acc))+'</span><strong>'+money(acc.sale?.total||0)+'</strong></label>').join('')+'</fieldset>').join('')+'<div class="demo-bar-dialog-status" data-dialog-status></div><div class="demo-bar-dialog-actions"><button class="rv2-btn" data-merge-cancel>Cancelar</button><button class="rv2-btn rv2-btn-primary" data-merge-confirm>Unir seleccionadas</button></div>';
