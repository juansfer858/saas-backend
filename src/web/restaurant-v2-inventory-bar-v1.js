...
function recipeIngredient(id){return S.recipes?.ingredients?.find(x=>x.id===id)||null}
function recipeProduct(id){return S.recipes?.products?.find(x=>x.id===id)||null}
function renderRecipes(){
  const data=S.recipes||{ingredients:[],products:[]},ings=data.ingredients||[],products=data.products||[],low=ings.filter(i=>i.active&&Number(i.available)<=Number(i.minimum));
  $('#recipeMetrics').innerHTML=[
    ['Ingredientes activos',ings.filter(i=>i.active).length,'materias primas'],
    ['Bajo mínimo',low.length,'requieren revisión'],
    ['Productos con receta',products.filter(p=>p.hasRecipe).length,'costeo configurado'],
    ['Costo teórico total',money(products.reduce((s,p)=>s+Number(p.recipeCost||0),0)),'suma de recetas']
  ].map(x=>'<article class="inv-metric"><span>'+esc(x[0])+'</span><b>'+esc(x[1])+'</b><small>'+esc(x[2])+'</small></article>').join('');
  $('#ingredientRows').innerHTML=ings.length?ings.map(i=>'<tr><td class="inv-product"><b>'+esc(i.name)+'</b><span>'+esc(i.code)+(i.active?'':' · Inactivo')+'</span></td><td>'+esc(i.unit)+'</td><td class="inv-num">'+money(i.unitCost)+'</td><td class="inv-num">'+q(i.onHand)+'</td><td class="inv-num inv-reserved">'+q(i.reserved)+'</td><td class="inv-num '+(i.available<=0?'inv-out':i.available<=i.minimum?'inv-low':'inv-ok')+'">'+q(i.available)+'</td><td class="inv-num">'+q(i.minimum)+'</td><td><div class="inv-row-actions"><button class="rv2-btn" data-ing-edit="'+esc(i.id)+'">Editar</button><button class="rv2-btn" data-ing-move="'+esc(i.id)+'">Movimiento</button><button class="rv2-btn" data-ing-min="'+esc(i.id)+'">Mínimo</button><button class="rv2-btn" data-ing-history="'+esc(i.id)+'">Historial</button></div></td></tr>').join(''):'<tr><td colspan="8"><div class="inv-empty">Crea ingredientes para comenzar.</div></td></tr>';
  $('#recipeProductRows').innerHTML=products.length?products.map(p=>'<tr><td class="inv-product"><b>'+esc(p.name)+'</b><span>'+esc(p.sku)+(p.hasRecipe?' · Receta activa':' · Sin receta')+'</span></td><td class="inv-num">'+money(p.price)+'</td><td class="inv-num">'+money(p.recipeCost)+'</td><td class="inv-num '+(p.margin>=0?'inv-ok':'inv-out')+'">'+money(p.margin)+'</td><td>'+esc((p.ingredients||[]).length)+' ingrediente(s)</td><td><button class="rv2-btn '+(p.hasRecipe?'':'rv2-btn-primary')+'" data-recipe-product="'+esc(p.id)+'">'+(p.hasRecipe?'Editar receta':'Configurar receta')+'</button></td></tr>').join(''):'<tr><td colspan="6"><div class="inv-empty">No hay productos de Carta.</div></td></tr>';
  $$('[data-ing-edit]').forEach(b=>b.onclick=()=>openIngredient(recipeIngredient(b.dataset.ingEdit)));
  $$('[data-ing-move]').forEach(b=>b.onclick=()=>openIngredientMove(recipeIngredient(b.dataset.ingMove)));
  $$('[data-ing-min]').forEach(b=>b.onclick=()=>openIngredientMin(recipeIngredient(b.dataset.ingMin)));
  $$('[data-ing-history]').forEach(b=>b.onclick=()=>openIngredientHistory(recipeIngredient(b.dataset.ingHistory)));
  $$('[data-recipe-product]').forEach(b=>b.onclick=()=>openRecipe(recipeProduct(b.dataset.recipeProduct)));
}
async function loadRecipes(show=true){if(show)notice('Cargando recetas y costos…');S.recipes=await V.api('/api/v1/restaurante/recetas-costos-v1');renderRecipes();if(show)notice('Recetas y costos actualizados.')}
function openIngredient(row=null){
  $('#ingredientId').value=row?.id||'';$('#ingredientTitle').textContent=row?'Editar ingrediente':'Nuevo ingrediente';$('#ingredientCode').value=row?.code||'';$('#ingredientName').value=row?.name||'';$('#ingredientUnit').value=row?.unit||'g';$('#ingredientCost').value=Number(row?.unitCost||0);$('#ingredientActive').checked=row?.active!==false;err('#ingredientError');$('#ingredientDialog').showModal()
}
function openIngredientMove(row){if(!row)return;$('#ingredientMoveId').value=row.id;$('#ingredientMoveVersion').value=row.stockVersion;$('#ingredientMoveTitle').textContent=row.name+' · '+q(row.available)+' '+row.unit+' disponibles';$('#ingredientMoveKind').value=row.onHand===0?'initial':'entry';$('#ingredientMoveQty').value='';$('#ingredientMoveCost').value=Number(row.unitCost||0);$('#ingredientMoveReason').value='';err('#ingredientMoveError');syncIngredientCost();$('#ingredientMoveDialog').showModal()}
function syncIngredientCost(){$('#ingredientCostWrap').hidden=$('#ingredientMoveKind').value!=='entry'}
function openIngredientMin(row){if(!row)return;$('#ingredientMinId').value=row.id;$('#ingredientMinVersion').value=row.stockVersion;$('#ingredientMinTitle').textContent=row.name;$('#ingredientMinValue').value=row.minimum;err('#ingredientMinError');$('#ingredientMinDialog').showModal()}
async function openIngredientHistory(row){if(!row)return;$('#costTitle').textContent='Historial · '+row.name;$('#costRows').innerHTML='<div class="inv-empty">Cargando…</div>';$('#costDialog').showModal();try{const rows=await V.api('/api/v1/restaurante/recetas-costos-v1/ingredientes/historial?ingredient='+encodeURIComponent(row.id));$('#costRows').innerHTML=rows.length?rows.map(x=>'<div class="inv-cost-row"><span>'+esc(dateText(x.creadoEn))+'</span><div><b>'+esc(kindLabel(x.kind))+'</b><br><span>'+esc(x.reason)+' · '+q(x.quantity)+' '+esc(row.unit)+'</span></div><strong>'+(x.delta>0?'+':'')+q(x.delta)+' → '+q(x.balance)+'</strong></div>').join(''):'<div class="inv-empty">Sin movimientos.</div>'}catch(e){$('#costRows').innerHTML='<div class="inv-error">'+esc(e.message)+'</div>'}}
function ingredientOptions(selected=''){return '<option value="">Selecciona ingrediente</option>'+(S.recipes?.ingredients||[]).filter(i=>i.active||i.id===selected).map(i=>'<option value="'+esc(i.id)+'" '+(i.id===selected?'selected':'')+'>'+esc(i.name)+' · '+esc(i.unit)+' · '+money(i.unitCost)+'</option>').join('')}
function addRecipeLine(seed={}){
  const id='rl'+(++S.recipeLines),wrap=document.createElement('div');wrap.className='inv-recipe-line';wrap.dataset.recipeLine=id;wrap.innerHTML='<label>Ingrediente<select class="inv-select" data-rl-ing>'+ingredientOptions(seed.ingredientId||'')+'</select></label><label>Cantidad por venta<input class="inv-input" data-rl-qty type="number" min="0.001" step="0.001" value="'+esc(seed.quantity||'')+'"></label><label>Costo línea<span class="inv-recipe-cost" data-rl-cost>$ 0</span></label><button class="rv2-btn" type="button" data-rl-remove>×</button>';$('#recipeLines').appendChild(wrap);$('[data-rl-remove]',wrap).onclick=()=>{wrap.remove();renderRecipeTotal()};$$('input,select',wrap).forEach(n=>{n.oninput=renderRecipeTotal;n.onchange=renderRecipeTotal});renderRecipeTotal()
}
function currentRecipeLines(){return $$('.inv-recipe-line').map(row=>({ingredientId:$('[data-rl-ing]',row)?.value||'',quantity:num($('[data-rl-qty]',row)?.value)})).filter(x=>x.ingredientId&&x.quantity>0)}
function renderRecipeTotal(){let total=0;$$('.inv-recipe-line').forEach(row=>{const ing=recipeIngredient($('[data-rl-ing]',row)?.value),amount=num($('[data-rl-qty]',row)?.value),line=amount*num(ing?.unitCost);total+=line;const n=$('[data-rl-cost]',row);if(n)n.textContent=money(line)});const product=recipeProduct($('#recipeProductId').value);$('#recipeTotal').innerHTML='<span>Costo teórico <b>'+money(total)+'</b></span><span>Precio <b>'+money(product?.price||0)+'</b></span><span>Margen bruto <b>'+money(num(product?.price)-total)+'</b></span>'}
function openRecipe(productRow){
  if(!productRow)return;$('#recipeProductId').value=productRow.id;$('#recipeTitle').textContent='Receta · '+productRow.name;$('#recipePrice').innerHTML='<span>Precio de venta</span><strong>'+money(productRow.price)+'</strong>';$('#recipeBuilder').textContent='Usa cantidades por una venta. Ejemplo: 180 g de carne, 40 g de queso, 1 unidad de pan.';$('#recipeLines').innerHTML='';S.recipeLines=0;(productRow.ingredients?.length?productRow.ingredients:[{}]).forEach(addRecipeLine);$('#clearRecipe').disabled=!productRow.hasRecipe;err('#recipeError');$('#recipeDialog').showModal();renderRecipeTotal()
}
function setTab(tab){S.tab=tab;$$('[data-inv-tab]').forEach(b=>b.classList.toggle('active',b.dataset.invTab===tab));$('#stockPanel').hidden=tab!=='stock';$('#purchasesPanel').hidden=tab!=='purchases';$('#recipesPanel').hidden=tab!=='recipes';$('#historyPanel').hidden=tab!=='history';if(tab==='purchases'&&!S.purchases.length)loadPurchases();if(tab==='recipes'&&!S.recipes)loadRecipes();if(tab==='history'&&!S.history.length)loadHistory()}
...
function bind(){
  $('#refresh').onclick=async()=>{try{await Promise.all([loadWorkspace(false),S.tab==='purchases'?loadPurchases(false):Promise.resolve(),S.tab==='recipes'?loadRecipes(false):Promise.resolve()]);if(S.tab==='history')await loadHistory();notice('Información actualizada.')}catch(e){notice(e.message,true)}};
  ...
  $('#newIngredient').onclick=()=>openIngredient();
  $('#ingredientMoveKind').onchange=syncIngredientCost;
  $('#ingredientForm').onsubmit=async e=>{e.preventDefault();err('#ingredientError');try{await V.api('/api/v1/restaurante/recetas-costos-v1/ingredientes',{method:'POST',body:JSON.stringify({id:$('#ingredientId').value||undefined,code:$('#ingredientCode').value.trim(),name:$('#ingredientName').value.trim(),unit:$('#ingredientUnit').value,unitCost:Number($('#ingredientCost').value||0),active:$('#ingredientActive').checked})});$('#ingredientDialog').close();await loadRecipes(false);notice('Ingrediente guardado.')}catch(x){err('#ingredientError',x.message)}};
  $('#ingredientMoveForm').onsubmit=async e=>{e.preventDefault();err('#ingredientMoveError');try{const kind=$('#ingredientMoveKind').value;await V.api('/api/v1/restaurante/recetas-costos-v1/ingredientes/movimiento',{method:'POST',body:JSON.stringify({id:$('#ingredientMoveId').value,version:Number($('#ingredientMoveVersion').value),kind,quantity:Number($('#ingredientMoveQty').value),unitCost:kind==='entry'?Number($('#ingredientMoveCost').value):undefined,reason:$('#ingredientMoveReason').value.trim()})});$('#ingredientMoveDialog').close();await loadRecipes(false);notice('Movimiento de ingrediente guardado.')}catch(x){err('#ingredientMoveError',x.message)}};
  $('#ingredientMinForm').onsubmit=async e=>{e.preventDefault();err('#ingredientMinError');try{await V.api('/api/v1/restaurante/recetas-costos-v1/ingredientes/'+encodeURIComponent($('#ingredientMinId').value)+'/minimo',{method:'PATCH',body:JSON.stringify({version:Number($('#ingredientMinVersion').value),minimum:Number($('#ingredientMinValue').value)})});$('#ingredientMinDialog').close();await loadRecipes(false);notice('Mínimo de ingrediente actualizado.')}catch(x){err('#ingredientMinError',x.message)}};
  $('#addRecipeIngredient').onclick=()=>addRecipeLine();
  $('#recipeForm').onsubmit=async e=>{e.preventDefault();err('#recipeError');try{const items=currentRecipeLines();if(!items.length)throw new Error('Agrega al menos un ingrediente válido.');await V.api('/api/v1/restaurante/recetas-costos-v1/productos/'+encodeURIComponent($('#recipeProductId').value),{method:'PUT',body:JSON.stringify({items})});$('#recipeDialog').close();await Promise.all([loadRecipes(false),loadWorkspace(false)]);notice('Receta guardada.')}catch(x){err('#recipeError',x.message)}};
  $('#clearRecipe').onclick=async()=>{const id=$('#recipeProductId').value,p=recipeProduct(id);if(!p||!p.hasRecipe)return;if(!confirm('¿Quitar la receta de '+p.name+'?'))return;try{await V.api('/api/v1/restaurante/recetas-costos-v1/productos/'+encodeURIComponent(id),{method:'DELETE'});$('#recipeDialog').close();await Promise.all([loadRecipes(false),loadWorkspace(false)]);notice('Receta retirada.')}catch(e){err('#recipeError',e.message)}};
  $('#exportRecipes').onclick=()=>{if(!S.recipes)return;downloadCsv('Recetas_costos_'+today()+'.csv',['Producto','Precio','Costo receta','Margen','Ingrediente','Cantidad','Unidad','Costo unitario','Costo línea'],(S.recipes.products||[]).flatMap(p=>(p.ingredients||[]).length?(p.ingredients||[]).map(i=>[p.name,p.price,p.recipeCost,p.margin,i.ingredientName,i.quantity,i.unit,i.unitCost,i.lineCost]):[[p.name,p.price,p.recipeCost,p.margin,'','','','','']]))};
...
async function boot(){
  ...
  try{await Promise.all([loadWorkspace(false),loadSuppliers()]);notice('Inventario listo.')}catch(e){notice(e.message||'No fue posible cargar Inventario.',true)}
}
