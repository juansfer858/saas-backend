...
(()=>{'use strict';
  ...
  const input=document.getElementById('menuSearch');const clear=document.getElementById('menuSearchClear');const list=document.getElementById('menuList');const empty=document.getElementById('menuSearchEmpty');
  if(!input||!list)return;
  const normalize=value=>String(value||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/\s+/g,' ').trim();
  let query='';let switching=false;
  function showAllCategory(){
    if(!query||switching)return;
    const all=document.querySelector('#categoryNav [data-category="TODO"]');if(!all||all.getAttribute('aria-selected')==='true')return;
    switching=true;all.click();queueMicrotask(()=>{switching=false;apply()});
  }
  function apply(){
    query=normalize(input.value);if(clear)clear.hidden=!query;
